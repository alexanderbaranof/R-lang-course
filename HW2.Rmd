...
Answer:
We can tell that it is not a normal distribution, we see a long tail on the right and a clear bias
...
Answer:
The current version is more like a normal distribution
...
mono1 %>% 
  ...
  geom_col(position = 'dodge', width=0.5) + facet_grid(~WordOrder)
```
